...
def main():
    ...
    # mygym 
    parser.add_argument('--edge_penalty', type=int, default=10)
    parser.add_argument('--width', type=int, default=5)
    parser.add_argument('--height', type=int, default=5)
    parser.add_argument('--easy', action='store_true')
    parser.add_argument('--nohole', action='store_true')
    parser.add_argument('--hidden_dist', type=int, default=0)
    parser.add_argument('--lstm', action='store_true')
    parser.add_argument('--hrlenv', action='store_true')
    parser.add_argument('--markenv', action='store_true')
    ...
    def make_env(process_idx, test):
        ...
        env = myenv.CliffGridEnv(process_idx=process_idx, edge_penalty=args.edge_penalty, width=args.width, height=args.height, easy=args.easy, nohole=args.nohole, hidden_dist=args.hidden_dist, mark_env=args.markenv, hrl_env=args.hrlenv, debug=args.debug)
    ...
    trial=optuna.trial.FixedTrial(params={'n_output_channels': 136, 'layer_type': 'CNN', 'n_layers': 3, 'n_channels_cnn': 128, 'optimizer': 'sgd', 'sgd.lr': 0.09315239055341833})
    n_actions = 4
    if args.lstm:
        model = myenv.MyA3CLSTM(trial, width=args.width, height=args.height, action_size=n_actions, lstm_size=128)
    else:
        model = myenv.MyModel(trial, n_actions, width=args.width, height=args.height)
